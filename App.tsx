...
import { Upload, Play, Download, Trash2, Layers, Search, X, FolderArchive, CheckSquare, Zap, Pause, Key, FileArchive } from 'lucide-react';
import { TaggedImage, AppSettings, DEFAULT_PROMPT } from './types';
...
const App: React.FC = () => {
  ...
  const handleDeleteSelected = () => {
      // 1. Calculate counts using current state closure for the confirm dialog
      ...
      if (window.confirm(`确定要删除选中的 ${count} 张图片吗？`)) {
          ...
          // 3. Update state using FUNCTIONAL update to ensure we have the LATEST state
          setImages(currentImages => {
              const toKeep: TaggedImage[] = [];
              currentImages.forEach(img => {
                  if (img.selected) {
                      // Release memory for deleted items
  ...
  const handleClearAll = () => {
    if (window.confirm("确定要清空所有图片吗？")) {
        ...
        // 2. Functional update for total reset
        setImages(currentImages => {
            // Clean up everything
